...
class Game:
    ...
    def __init__(self, num_players):
        ...
        self.the_winner, self.capitalist_count, self.worker_count = self.play() #Initiating the class starts game play
    ...
    def play(self):
        ...
        while self.the_end is False:
            ...
            for player in player_list:
                # Check for end state in case you're mid-turn
                if self.the_end is True:
                    break
                # Start player turn
                print("-"*50)
                print(f"{player.player_type.title()}'s turn! Start Position: {player.position}")
                ...
                if (player.position + roll) > board.end:
                    ...
                    print(f"{player.player_type.title()}'s turn over! End Position: {player.position}\n")
                    continue
                ...
                self.get_stats(player)
                print(f"\n{player.player_type.title()}'s turn over! End Position: {player.position}")
            # Check for end state again
            if self.the_end is True:
                break
    ...
    def pull_card(self, board, player, other_player):
        ...
        card_action = board.chance_cards[player.player_type][player.card_count]["action"]
        card_count = board.chance_cards[player.player_type][player.card_count]
        # Asset card
        if card_action == "asset":
            change = int(card_count["asset"])
            player.assets += change
            print(f"Your assets increase by {change}.\n")   
        # Debit card
        elif card_action == "debit":
            change = int(card_count["debit"])
            player.debits += change
            print(f"Your debits increase by {change}.\n")  
        # Position card
        elif card_action == "position":
            change = int(card_count["position"])
            if (player.position + change) == board.end: 
                self.the_end = True
                self.the_winner = player
                player.position += change
                print(f"THE {player.player_type.upper()} WINS!\n")
            elif (player.position + change) < board.end and (player.position + change) > 0:
                player.position += change
        ...
        # Turn card
        elif card_action == "turn":
            change = int(card_count["turn"])
            if change < 0:
            ...
            player.turn += change
        # Confrontation card
        elif card_action == "confrontation":
    ...
    def confrontation(self, player, other_player):
        ...
        # For the purposes of the demo, the confrontation "GO" condition is when the 
        # player has more assets than debits. 
        print("YOU HAVE DRAWN A CONFRONTATION!")
        print("WINNING THE CONFRONTATION SECURES")
        print("THE VICTORIOUS CLASS TWO FREE THROWS OF THE DICE!\n")        
        if player.assets > player.debits:
            ...
            if player_total > other_player_total:
                print("You win this Confrontation. You earn 2 extra turns.\n")
                player.turn += 2
            elif player_total < other_player_total:
                print("You lose this Confrontation. Other player earns 2 extra turns.\n")
                other_player.turn += 2
            elif player_total == other_player_total:
                print("This confrontation is at a stalemate! Nothing happens.\n")
        else:
            print("You do not have an assets advantage. Better to sit this one out.\n") 
...
class Board(Game):
    ...
    Attributes:
        ...
        chance_cards: 3-level dictionary that contains both the capitalist and worker chance cards
    ...
    def create_board(self):
        ...
        worker_cards_csv = open("worker_chance_cards.csv","rt",encoding="utf-8")
        ...
        # FIRST, READ THE CSV FILE + APPEND TO LIST
        capitalist_cards_csv = open("capitalist_chance_cards.csv", "rt", encoding="utf-8")
